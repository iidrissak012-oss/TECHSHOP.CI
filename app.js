...
// ====== PRODUITS PAR DÉFAUT (fallback si Supabase indisponible) ======
const DEFAULT_PRODUCTS = [
  ...
  { id:15, name:'Support Voiture Magnét.',   cat:'accessories',   price:6000,   brand:'Baseus',    image:'https://images.unsplash.com/photo-1617814076367-b759c7d7e738?auto=format&fit=crop&w=900&q=80', description:'Magnétique 360°, compatible MagSafe', available:true },
...
function initSupabase() {
  try {
    if (window.supabase) {
      // Pas de vérification de placeholder — les vraies clés sont là
      sb = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
      console.log('✅ Supabase client connecté');
    } else {
      console.warn('⚠️ SDK Supabase non disponible, mode fallback');
    }
  } catch(e) {
  ...
  try {
    if (sb) {
      ...
      if (!error && data && data.length > 0) {
        products = data;
        // Charger le numéro WA depuis Supabase
        try {
          const { data: waData } = await sb.from('settings').select('value').eq('key','wa_number').maybeSingle();
          if (waData?.value) waNumber = waData.value;
        } catch(e) { /* ignore */ }
        return;
      }
      if (error) console.warn('Supabase error:', error.message);
    }
    // Fallback localStorage ou produits par défaut
  ...
  card.innerHTML =
    ...
    '<div class="product-body">' +
      ...
      (product.available !== false
        ? '<a href="' + waLink + '" target="_blank" class="btn-wa-product">' +
...
function renderFeaturedProducts() {
  ...
  if (!c) return;
  c.innerHTML = '';
  products.filter(p => p.available !== false).slice(0, 8)
    .forEach(p => c.appendChild(createProductCard(p)));
}
...
function renderBoutiqueProducts() {
  ...
  c.innerHTML = '';
  if (!list.length) {
    c.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:60px;color:#718096">' +
      '<div style="font-size:3rem;margin-bottom:16px">🔍</div><h3>Aucun produit trouvé</h3></div>';
    return;
  }
...
async function submitRepairForm() {
  ...
  const name    = v('rep-name');
  const phone   = v('rep-phone');
  const brand   = v('rep-brand');
  const model   = v('rep-model');
  const type    = v('rep-type');
  const desc    = v('rep-desc');
  ...
  if (sb) {
    try {
      await sb.from('repairs').insert([{ name, phone, brand, model, type, description: desc, dropoff }]);
    } catch(e) { console.warn('Erreur save repair:', e); }
  }

  const msg =
    '*DEMANDE DE RÉPARATION — TechShop CI*\n\n' +
    '👤 *Nom :* ' + name + '\n' +
    '📞 *Téléphone :* ' + phone + '\n' +
